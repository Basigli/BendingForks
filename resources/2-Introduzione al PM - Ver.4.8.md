...
- Facebook è un ottimo esempio di un’ottima idea e un’ottima execution:
  ...
  - Facebook aveva una soluzione che interessava molto il pubblico.

  - L’infrastruttura funzionava, scalava e offriva un’ottima UX.
...
- Un prodotto è qualcosa di più di un programma:
  ...
  - deve essere adeguatamente testato (la probabilità di incorrere in

un malfunzionamento deve essere la più bassa possibile);
    - può essere utilizzato in diversi ambienti e con molti set di dati;
    - può essere eseguito, testato e modificato da chiunque;
    - la documentazione deve essere dettagliata e completa.
...
seguenti requisiti:
  ...
  - la modalità di integrazione dei componenti deve essere definita

con precisione;

    - input e output devono rispettare dei tracciati ben precisi, che
definiscono le interfacce di comunicazione tra i componenti;

    - deve essere testata l’integrazione tra i diversi componenti;
    - alcune risorse disponibili sono limitate.
...
- I motivi principali possono essere ricercati nei seguenti fattori:
  ...
  - Mancato rispetto degli obiettivi (integrità concettuale violata);
  - Difficoltà nella stima corretta dei tempi (e.g., troppo spesso lo

sviluppatore sottostima volutamente il tempo per compiacere il
management);

    - Troppo ottimismo (i.e., lo sviluppatore non pensa mai che le cose

potrebbero andare male);

    - La convinzione che l’impegno da solo assicuri l’avanzamento del

progetto (Effort ≠Progress);

    - Mancato monitoraggio dello stato di avanzamento rispetto a quanto
...
- Le “gioie” del realizzare un software possono essere le seguenti:
  ...
  - la soddisfazione di fare applicazioni che altre persone useranno;

  - il fascino di risolvere rompicapi complessi;

  - trovare uno stimolo continuo per la propria curiosità e desiderio

di imparare;

    - lavorare con un “mezzo” molto flessibile;
...
- I “dolori” del realizzare un software possono essere i seguenti:
  ...
  - spesso non c’è controllo di ciò che deve essere prodotto (si dipende

dalle specifiche);

    - spesso si deve dipendere da altri programmi e/o programmatori;

    - il debug (spesso scovare e correggere bugs è difficile e frustrante);

    - quando si termina un progetto solitamente il prodotto è già
...
- Prenderemo in considerazione due proposte molto alternative:
  ...
  - Un team molto “tradizionale”, che prevede numerose figure

molto specializzate.

    - Un team molto “agile”, dove sono definite poche figure e in cui
...
- I pericoli per gli architetti:
  ...
  - Le specifiche degli architetti potrebbero essere troppo “ricche” e non

considerare in modo adeguato i costi di implementazione.

    - Gli architetti si prendono tutto il divertimento lasciando pochi margini
...
“what” e “how” l’implementazione può essere divertente.
    ...
    - Gli sviluppatori devono essere coinvolti quando le specifiche sono
...
- Documentare il proprio lavoro è fondamentale per:
  ...
  - indicare il funzionamento di un programma, un componente, etc.;
  - ricordarsi a distanza di tempo cosa si è fatto;
  - condividere il proprio lavoro con i colleghi;
  - definire formalmente le interfacce tra componenti, etc.;
  - tenere traccia dei problemi e delle soluzioni (e.g., bugs, etc.);
  - stabilire i test case, definire il loro utilizzo, etc.;
  - etc...
...
- Brooks individua i seguenti quesiti a cui bisogna dare una risposta:
  ...
  - Environment: Su quale macchina, configurazione e sistema operativo

girerà il software?

    - Domain and Range: Quale dominio dell’input è valido? Quale output

si ritiene ammissibile?

    - Function Realized and Algorithm Used: Precisamente cosa permette

di fare il software e come?

    - Input-Output Formats: Qual è il formato e il mezzo per effettuare

l’input e output? In questo caso la descrizione deve essere completa
e precisa.

    - Operating Instruction: Come si usa? Quali sono le eccezioni e come

si gestiscono?

    - Options: Quali scelte può effettuare l’utente? Come queste scelte

devono essere specificate?

    - Running Time: Quanto tempo impiega il software per eseguire una
funzionalità su un input di una certa dimensione e su una macchina
con una specifica configurazione?

    - Accuracy and Checking: Quanto è precisa la soluzione fornita dal
...
individuare i seguenti elementi/strumenti:
    - Rappresentazione grafica della struttura di ciascun componente (i.e., flow

charts, etc.).

    - Strutture dati utilizzate e definizione degli input e output.
    - Completa descrizione degli algoritmi utilizzati nell’implementazione, con

eventuali riferimenti bibliografici, etc.

    - Descrizione delle tecnologie utilizzate e dettagli sulle modalità di utilizzo.
    - Discussione dei possibili miglioramenti che si possono apportare, dei difetti e
...
seguente documentazione:
    ...
    - What: objective. Questo documento definisce i bisogni che devono
essere soddisfatti, gli obiettivi, i desiderata, i vincoli e le priorità.

    - What: product specifications. La prima versione del documento può
...
e una descrizione dettagliata dell’implementazione (a uso interno).
    ...
    - When: schedule. Descrizione dell’organizzazione di tutte le attività di
progetto (i.e., GANTT, etc.). Può prevedere l’uso di strumenti software
specifici (e.g., Microsoft Project, etc.).

    - How much: budget. Questa documentazione è tra le più importanti
per la gestione dei progetti. Il budget può influenzare anche le scelte
tecniche.

    - Where: space allocation. Solitamente nella produzione del software
non è un tema fondamentale, però non deve essere sottovalutato e
deve essere considerato.

    - Who: organization chart. Documentare l’organizzazione delle risorse
...
1. La documentazione serve per comunicare le decisioni agli altri.
...
1. Deliver working software frequently, from a couple of weeks to a
...
1. At regular intervals, the team reflects on how to become more
